...
<body>
    ...
    <div class="container">
      ...
      <form action="{{url('/editPaymentSave')}}" method='post'>
      ...
      @csrf
        ...
        <select id="type" name="type" value='{{$data->type}}' ><br>
          ...
          <input type="text" id="cashier" name="amount" value='{{$data->amount}}' ><br>
          <span style='color:red'>@error('amount'){{$message}}@enderror</span><br>

          <label for="cashier">{{__('field.cashier')}}</label><br>
          <input type="text" id="cashier" name="cashier"  value='{{$data->cashier}}' ><br>
          <span style='color:red'>@error('cashier'){{$message}}@enderror</span><br>

          <label for="sourceOfPayment">{{__('field.waysOfPayment')}}</label><br>
         <input type="text" name="sourceOfPayment" value='{{$data->waysOfPayment}}'><br>
          <span style='color:red'>@error('sourceOfPayment'){{$message}}@enderror</span><br>

          <label for="account">{{__('field.acountID')}}</label><br>
          <input type="text" id="AccountID" name="accountID" value='{{$data->accountID}}'><br>
